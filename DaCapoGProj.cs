...
namespace DaCapo
{
    public class DaCapoGProj : GlobalProjectile
    {
					...
					if (Main.npc[i].active && !Main.npc[i].dontTakeDamage)
					{
						...
						if (flag2 == null || flag2.Value)
						{
							...
							if (flag3 == null || flag3.Value)
							{
								...
								if ((projectile.hostile && !projectile.friendly && Main.npc[i].HasBuff(ModContent.BuffType<FerventAdoration>()) && !Main.npc[i].dontTakeDamage) && Main.npc[i].immune[255] == 0)                 //重点判定
								{
									bool flag6 = false;
									...
									if (!flag6 && (Main.npc[i].noTileCollide || !projectile.ownerHitCheck || projectile.CanHit(Main.npc[i])))
									{
										...
										if (flag7)
										{
											...
											{
												float num21 = (float)Math.Sqrt((double)(projectile.velocity.X * projectile.velocity.X + projectile.velocity.Y * projectile.velocity.Y));
												if (num21 < 1f)
												{
													num21 = 1f;
												}
